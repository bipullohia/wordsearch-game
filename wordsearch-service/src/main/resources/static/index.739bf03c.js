...
async function fetchWordGridFromService() {
    ...
    if (wordInputDisplaySection.lastChild) wordInputDisplaySection.removeChild(wordInputDisplaySection.lastChild);
...
class Grid {
    ...
    getCellsInRange(firstLetter, currentLetter) {
        ...
        if (firstLetter.x > currentLetter.x || firstLetter.y > currentLetter.y) [firstLetter, currentLetter] = [
            currentLetter,
            firstLetter
        ];
        //horizontal selection
        if (firstLetter.x === currentLetter.x) for(let i = firstLetter.y; i <= currentLetter.y; i++)cellsInRange.push(this.gridSection.querySelector(`td[data-x="${currentLetter.x}"][data-y="${i}"]`));
        else if (firstLetter.y === currentLetter.y) for(let i = firstLetter.x; i <= currentLetter.x; i++)cellsInRange.push(this.gridSection.querySelector(`td[data-x="${i}"][data-y="${currentLetter.y}"]`));
        else if (firstLetter.y - currentLetter.y === firstLetter.x - currentLetter.x) {
            let diff = currentLetter.y - firstLetter.y;
            for(let i = 0; i <= diff; i++)cellsInRange.push(this.gridSection.querySelector(`td[data-x="${firstLetter.x + i}"][data-y="${firstLetter.y + i}"]`));
        }
        return cellsInRange;
    }
    renderGrid(gridSize, gridContent) {
        ...
        if (gridSection.lastChild) gridSection.removeChild(gridSection.lastChild);
        ...
        tbl.addEventListener("mouseup", ()=>{
            ...
            else if (this.words.indexOf(inverseSelectedWord) !== -1) this.foundWords.push(inverseSelectedWord);
            else this.selectedItems.forEach((cell)=>{
...
},{}]},["gHDMI","ebWYT"], "ebWYT", "parcelRequire94c2")
